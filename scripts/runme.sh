...
curl -sL "https://api.github.com/repos/ghdlsvc/dl21/releases" | grep helmiwrt-18.06-k5.4-"${1}" |  sed -e 's|\"||g'  -e  's/://g' | awk -F 'browser_download_url' '{print$2}' | sed -e '/^[[:space:]]*$/d' > ${anu}
